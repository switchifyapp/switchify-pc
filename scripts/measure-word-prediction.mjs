...
const [executable, app, expectedWord, lookupPath] = process.argv.slice(2);
if (!executable || !app || !expectedWord) {
  console.error('Usage: node scripts/measure-word-prediction.mjs <executable> <app-label> <expected-word> [lookup-path]');
  process.exit(1);
}
...
const child = spawn(resolve(executable), [
  ...
  resolve(lookupPath ?? 'src-tauri/resources/word-predictions.lookup'),
  '[]',
...
async function query(generation) {
  ...
  const bytes = Buffer.from(JSON.stringify({ Query: {
    generation, edits: [], revision: 0, shift: false, caps: false,
  } }));
...
let firstRequestMs;
let firstResponseFromSpawnMs;
let firstSuggestionsFromSpawnMs;
let memoryBytes;
let memoryMetric;
try {
  ...
  for (let generation = 0; generation < 200 && times.length < 100; generation++) {
    ...
    if (generation === 0) {
      ...
      firstResponseFromSpawnMs = performance.now() - spawnedAt;
    }
    ...
    if (result.batch?.words?.length) {
      firstSuggestionsFromSpawnMs ??= performance.now() - spawnedAt;
  ...
  try {
    if (process.platform === 'darwin') {
      memoryBytes = Number(execFileSync('ps', ['-o', 'rss=', '-p', String(child.pid)], { encoding: 'utf8' }).trim()) * 1024;
      memoryMetric = 'residentBytesAtEnd';
    } else if (process.platform === 'win32') {
      memoryBytes = Number(execFileSync('powershell.exe', ['-NoProfile', '-Command', `(Get-Process -Id ${child.pid}).PeakWorkingSet64`], { encoding: 'utf8', windowsHide: true }).trim());
      memoryMetric = 'peakWorkingSetBytes';
    }
  } catch { memoryBytes = undefined; }
  child.kill();
...
console.log(JSON.stringify({
  app, firstResponseFromSpawnMs, firstSuggestionsFromSpawnMs, memoryBytes, memoryMetric, successfulSamples: times.length, unsupported, failures, fixtureMatches,
