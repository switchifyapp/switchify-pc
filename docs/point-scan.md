...
The action menu uses a fixed grid of square icon tiles with labels beneath the artwork, on the same dark rounded panel chrome as the scanning keyboard. A yellow border and amber background identify the current row or item. The same artwork is drawn on Windows and macOS.
