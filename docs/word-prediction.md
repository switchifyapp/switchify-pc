...
The model loads in the worker while keyboard input remains available. Suggestions are blank until loading finishes. If loading or inference fails, or a call exceeds one second, the keyboard shows “Predictions unavailable” and continues accepting input; reopening it retries. A 400 ms search budget bounds candidate exploration. A clipped buffer with no complete earlier word yields no suggestions.

Candidates contain ASCII letters and apostrophes, must have sufficient model probability, and cannot appear in the blocklist. Single-letter candidates are limited to “a” and “I”. Up to five distinct suggestions are shown. Ordinary words use lowercase; mixed-case names retain model casing. Shift, Caps and sentence capitalization are applied when the keyboard displays a candidate.
