...
Both spike apps launched in the VMs and displayed Ready/input access ready using temporary manual-scan bindings. A disposable TextEdit file accepted ordinary `x` input. Automated Space attempts did not produce a visible scan, so suggestion selection, repeated switch characters, focus/selection changes, protected-field suppression, and stable live suggestion-row timing remain **unqualified** in this run. In Windows, the computer-control path did not reliably deliver even unrelated typing or clicks to the disposable Notepad document, including after Switchify was stopped. No conclusion that this is a provider or capture regression can be drawn from these attempts. Do not mark the interactive gate passed.
