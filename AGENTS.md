...
- Choose the milestone from the **next** open release after the current shipped version. Treat `package.json` / `src-tauri/Cargo.toml` and the latest GitHub release/tag as the shipped version. Do not put new work on a milestone whose version is already released, even if that milestone is still open. If the user names a target RC, use that open milestone instead.
...
- Close tracking or epic issues when every scoped child issue/PR they list is merged or closed. Do not leave empty checklists on shipped work.
